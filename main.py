...
DATABASE_URL = _env("DATABASE_URL").replace("postgres://", "postgresql+asyncpg://", 1)
...
async def add_transaction(
    ...
    """
    1C jo'natilmagan tranzaksiyalar massivini yuboradi.
    Kalit nomlari РегистрСведений.CashbackTransactions ga qarab moslanadi —
    hozircha bir necha variant qabul qilinadi.
    """
    delivered: list[str] = []
    ...
    for item in payload:
        ...
        if exists:
            delivered.append(ext_id)  # idempotent: qayta yuborilsa ham OK
            continue

        card = str(item.get("card_number") or item.get("КартаРаками") or "").strip()
        purchase = Decimal(str(item.get("purshace_amount") or item.get("purchase_amount") or 0))
        spent = Decimal(str(item.get("cashback_spent") or 0))
        earned = Decimal(str(item.get("amount") or 0))

        client = await session.scalar(select(Client).where(Client.card_number == card))
        if client is None:
            log.warning("Tranzaksiya %s: karta %s topilmadi", ext_id, card)
            continue

        if not earned and purchase:
            earned = (purchase * client.cashback_percentage / 100).quantize(Decimal("0.01"))
        ...
        delivered.append(ext_id)

    await session.commit()
    log.info("Tranzaksiya qabul qilindi: %s ta", len(delivered))
    return ok({"delivered": delivered, "count": len(delivered)})
